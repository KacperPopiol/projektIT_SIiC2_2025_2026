...
/**
 * Archiwizacja konwersacji (prywatnej lub grupowej)
 */
exports.archiveConversation = async (req, res) => {
  try {
    ...
    // Sprawdź czy to konwersacja prywatna
    const participant = await db.ConversationParticipant.findOne({
      where: {
        conversation_id: conversationId,
        user_id: userId,
    ...
    if (participant) {
      // Archiwizuj konwersację prywatną
    ...
    // Sprawdź czy to konwersacja grupowa
    const conversation = await db.Conversation.findByPk(conversationId);

	console.log('conversation type:', conversation ? conversation.conversation_type : 'not found');
    ...
    if (conversation && conversation.conversation_type === "group") {
      ...
      if (groupMember) {
        // Archiwizuj konwersację grupową
        ...
        return res.json({
          success: true,
          message: "Konwersacja grupowa zarchiwizowana",
        });
      }
    }
...
/**
 * Przywrócenie konwersacji z archiwum (prywatnej lub grupowej)
 */
  ...
  try {
    ...
    if (conversation && conversation.conversation_type === "group") {
      ...
      if (groupMember) {
        ...
        return res.json({
          success: true,
          message: "Konwersacja grupowa przywrócona z archiwum",
        });
      }
    }
...
exports.getConversations = async (req, res) => {
  try {
    ...
    if (includeArchived === "false") {
      groupWhereClause.is_archived = false;
    }
...
exports.toggleDisappearingMessages = async (req, res) => {
  try {
    ...
    if (
      conversation.disappearing_messages_enabled &&
      conversation.disappearing_messages_enabled_by
    ) {
      const enabledByUser = await db.User.findByPk(
        conversation.disappearing_messages_enabled_by,
        {
      ...
      if (
        enabledByUser &&
        typeof enabledByUser.default_disappearing_time === "number"
      ) {
    ...
    res.json({
      success: true,
      message: enabled
        ? "Tryb znikających wiadomości włączony"
        : "Tryb znikających wiadomości wyłączony",
      settings: {
        disappearing_messages_enabled:
          conversation.disappearing_messages_enabled,
        disappearing_messages_enabled_at:
          conversation.disappearing_messages_enabled_at,
...
exports.setConversationTheme = async (req, res) => {
  try {
    ...
    res.status(statusCode).json({
      error:
        statusCode === 500
          ? "Błąd serwera podczas ustawiania motywu"
          : error.message,
    });
  }
};
